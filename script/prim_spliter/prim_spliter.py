...
def load( data ):
    ...
    if geo.findPrimAttrib( data ):
        render = hou.ui.displayMessage('You want render setting??', 
                                       buttons=('OK', 'Cancel'), 
                                       title='Render setting')

        max_value = max(prim.attribValue(data) for prim in geo.prims())
        ...
        if max_value:
            ...
            warning_window(f'prim cluster is "{max_value}".')

    else:
        warning_window(f'"{data}" prim attrib is not exists.')
